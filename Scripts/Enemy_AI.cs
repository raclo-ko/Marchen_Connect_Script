...
public class Enemy_AI : AI
{
    ...
    void Start()
    {
        ...
        friends = new List<Enemy_AI>();
        GameObject[] go = GameObject.FindGameObjectsWithTag("Enemy");
        foreach(GameObject g in go)
        {
            Enemy_AI a=g.GetComponent<Enemy_AI>();
            if(a!=null) friends.Add(a);
        }
    ...
    {
        foreach(Enemy_AI f in friends)
        {
            if (f == null)
                continue;
            Vector2 vec = (f.gameObject.transform.position - playerpos);
            if (vec.magnitude <= _sight)
            {
    ...
    private bool isBlocked()
    {
        ...
        RaycastHit2D hit = Physics2D.Raycast(_col2.ClosestPoint(position + controller.pm.Facing * Vector3.right * _sight), controller.pm.Facing * Vector2.right, 0.2f);
