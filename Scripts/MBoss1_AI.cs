...
public class MBoss1_AI : AI
{
    ...
    void Start()
    {
        ...
        _skill3 = (Skill)gameObject.AddComponent(System.Type.GetType(ctrl.stat.Skill3));
    }
    ...
    private bool CheckSkill(Vector2 vec)
    {
        ...
        if (ctrl.curHp > 18 && ctrl.curHp <= 42 && skillcount == 0)
        {
            skillcount = 1;
            controller.inputMove = Vector2.zero;
            if (_skill3.doSkill()) return true;
        }
        if (ctrl.curHp <= 18 && skillcount <= 1)
        {
            skillcount = 2;
            controller.inputMove = Vector2.zero;
            if (_skill3.doSkill()) return true;
        ...
        if (_skill1.isCasting || _skill2.isCasting || _skill3.isCasting)
            return true;
